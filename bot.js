...
process.on('uncaughtException', (err) => {
  ...
  // process.exit(1) YAPILMADI – Render tekrar denesin
});
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (client.guilds.cache.has(invite.guild?.id)) {
        console.log(`Zaten ${invite.guild?.name || 'bilinmeyen'} sunucusunda → katılma atlanıyor`);
        return true;
      }
  ...
  // 2. Sunucu mesajları → bildirim kanalı
    ...
    if (message.channel.id === NOTIFICATION_CHANNEL_ID) {
      if (message.content.includes(TARGET_ROLE_MENTION)) {
        ...
        // Rolü yoksa → 3 sn sonra dm gel (senin son versiyonda 3000 ms idi)
        setTimeout(async () => {
          try {
...
client.once('ready', () => {
  ...
  // Render'ın "exited early" dememesi için her 5 dakikada bir log
  setInterval(() => {
