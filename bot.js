...
async function anaIsleyici(soru, kullaniciId, karakterAdi) {
  const suAn = new Date().toLocaleString('tr-TR', { timeZone: 'Europe/Istanbul' });
  const systemPrompt = `Sen ${karakterAdi}'sin. Geliştiricin Batuhan. Güncel Tarih: ${suAn}. Türkçe konuş.`;
  ...
  const sonCevap = cevap || (karakterAdi === 'Awe' ? 'Enerjim bitti...' : 'Simya enerjim düşük.');
...
async function banIhlalci(guild, userId, sebep) {
  try {
    await guild.members.ban(userId, { reason: `[Guard] ${sebep}` });
...
// Botları başlat
TOKENS.forEach(config => startBot(config));